...
import (
	...
	"github.com/kotlin2018/pkg/intlog"
)
...
func Database(name ...string) orm.DB {
	...
	db := instances.GetOrSetFuncLock(instanceKey, func() interface{} {
		...
		if len(configMap) == 0 && !orm.IsConfigured() {
			if !Config().Available() {
		...
		if node := parseDBConfigNode(configMap); node != nil {
			...
			if len(cg) > 0 {
				if orm.GetConfig(group) == nil {
					intlog.Printf("add configuration for group: %s, %#v", orm.DefaultGroupName, cg)
					orm.SetConfigGroup(orm.DefaultGroupName, cg)
				} else {
					intlog.Printf("ignore configuration as it already exists for group: %s, %#v", orm.DefaultGroupName, cg)
					intlog.Printf("%s, %#v", orm.DefaultGroupName, cg)
				}
			}
		}
		// Create a new ORM object with given configurations.
		if db, err := orm.New(name...); err == nil {
			if Config().Available() {
...
func parseDBConfigNode(value interface{}) *orm.ConfigNode {
